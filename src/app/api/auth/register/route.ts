...
export async function POST(req: Request) {
  try {
    ...
    if (error instanceof Error && (error as Error & { name: string }).name === "ValidationError") {
      const messages = Object.values((error as Error & { errors: Record<string, { message: string }> }).errors).map(
