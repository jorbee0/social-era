...
export default function CaseStudyForm() {
    ...
    const handleAddPoint = (section: 'challenge' | 'strategy' | 'after', field: 'points' | 'metrics') => {
    ...
    const handleRemovePoint = (section: 'challenge' | 'strategy' | 'after', field: 'points' | 'metrics', index: number) => {
        const newData = { ...formData };
        if (section === 'after' && field === 'metrics') {
    ...
    const handlePointChange = (section: 'challenge' | 'strategy' | 'after', field: 'points' | 'metrics', index: number, value: string) => {
        const newData = { ...formData };
        if (section === 'after' && field === 'metrics') {
    ...
    if (fetching) return <div className="flex items-center justify-center py-20"><Loader2 className="animate-spin w-10 h-10 text-brand-primary" /></div>;

    return (
        <form onSubmit={handleSubmit} className="space-y-10 pb-20">
            {/* Header */}
            <div className="flex justify-between items-center">
                <Link href="/admin/case-studies" className="flex items-center space-x-2 text-slate-400 hover:text-brand-primary transition-colors text-sm font-bold uppercase tracking-widest">
                    ...
                    <section className="bg-white p-10 rounded-3xl border border-slate-200 shadow-sm space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-3">
                                <label className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-500 ml-1">Study Title</label>
                                <input
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-500 ml-1">Industry Category</label>
                                <input
                                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-5 py-3.5 focus:ring-2 focus:ring-brand-primary/10 focus:border-brand-primary transition-all font-semibold"
                        ...
                        <div className="space-y-3">
                            <label className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-500 ml-1">Case Image URL</label>
                            <input
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-5 py-3.5 focus:ring-2 focus:ring-brand-primary/10 focus:border-brand-primary transition-all font-semibold"
                            ...
                            {formData.challenge.points.map((point, i) => (
                                <div key={i} className="flex gap-4">
                                    <input
                                        ...
                                        value={point}
                                        onChange={(e) => handlePointChange('challenge', 'points', i, e.target.value)}
                                        placeholder="Describe the challenge..."
                                    />
                                    <button type="button" onClick={() => handleRemovePoint('challenge', 'points', i)} className="text-slate-300 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                                </div>
                            ))}
                        </div>
                    </section>
                            ...
                            {formData.strategy.points.map((point, i) => (
                                <div key={i} className="flex gap-4">
                                    <input
                                        className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-5 py-3 text-sm font-medium"
                                        value={point}
                                        onChange={(e) => handlePointChange('strategy', 'points', i, e.target.value)}
                                        placeholder="Describe the action taken..."
                                    />
                                    <button type="button" onClick={() => handleRemovePoint('strategy', 'points', i)} className="text-slate-300 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                                </div>
                            ))}
                        </div>
                    </section>
                            ...
                            {formData.after.metrics.map((metric, i) => (
                                <div key={i} className="flex gap-4">
                                    <input
                                        ...
                                        value={metric}
                                        onChange={(e) => handlePointChange('after', 'metrics', i, e.target.value)}
                                        placeholder="e.g. +300% Growth"
                                    />
                                    <button type="button" onClick={() => handleRemovePoint('after', 'metrics', i)} className="text-white/20 hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
                                </div>
                            ))}
                        </div>
                    </section>
                    ...
                    <section className="bg-white p-10 rounded-3xl border border-slate-200 shadow-sm">
                        <div className="flex items-center justify-between">
                            <div className="space-y-1">
                                <h4 className="text-sm font-bold text-slate-800 uppercase tracking-widest">Visibility</h4>
                                <p className="text-xs text-slate-400 font-medium tracking-wide">Publish entry immediately</p>
                            </div>
                            <button
                                type="button"
                            ...
                            >
                                <div className={`w-6 h-6 bg-white rounded-full shadow-sm transform transition-transform ${formData.isPublished ? 'translate-x-6' : 'translate-x-0'}`} />
                            </button>
                        </div>
                    </section>
                    ...
                    {formData.image && (
                        <div className="rounded-3xl overflow-hidden aspect-[4/3] border border-slate-200 shadow-sm group relative">
                            ...
                            <div className="absolute inset-0 bg-brand-dark/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                <span className="text-white text-[10px] font-black uppercase tracking-[0.2em]">Live Preview</span>
                            </div>
                        </div>
                    )}
                </div>
