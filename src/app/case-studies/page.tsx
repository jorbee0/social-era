...
const CaseStudyBlock = ({ study, index }: { study: CaseStudy, index: number }) => {
    ...
    return (
        <section className={`py-24 ${isEven ? 'bg-white dark:bg-[#0A0D14]' : 'bg-slate-50 dark:bg-[#0F131C]'} transition-colors duration-500`}>
            <div className="container-custom">
                <div className={`flex flex-col ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'} gap-16 lg:gap-24 items-center`}>
                    {/* Content */}
                    <div className="w-full lg:w-1/2">
                        <span className="text-brand-primary font-bold text-xs uppercase tracking-[0.2em] mb-4 block">{study.industryCategory}</span>
                        <h2 className="text-3xl lg:text-4xl font-bold text-brand-dark dark:text-white mb-10 font-outfit tracking-tight">{study.title}</h2>
                            ...
                            <div>
                                ...
                                <ul className="space-y-3">
                                    {study.challenge.points.map((p, i) => (
                                        <li key={i} className="text-text-muted dark:text-white/60 font-inter flex items-start text-sm">
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                    {study.after.metrics.map((m, i) => (
                                        <div key={i} className="flex flex-col">
                                            <span className="text-white text-lg font-bold">{m.split(' ')[0]}</span>
                                            <span className="text-white/60 text-[10px] uppercase font-bold tracking-widest">{m.split(' ').slice(1).join(' ')}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="w-full lg:w-1/2 relative group">
                        <div className="relative aspect-[4/3] rounded-2xl overflow-hidden border border-slate-200 dark:border-white/10 shadow-2xl">
                            <img src={study.image} className="w-full h-full object-cover transform transition-transform duration-700 group-hover:scale-105" alt="Case Study Visual" />
        ...
        <main className="bg-white dark:bg-[#0A0D14] transition-colors duration-500">
            ...
            <section className="pt-32 pb-16 bg-white dark:bg-[#0A0D14]">
                <div className="container-custom">
                    ...
                    <p className="text-xl text-text-muted dark:text-white/60 leading-relaxed max-w-2xl font-inter animate-fade-in-up delay-100">
                        Proven Results. Structured Growth. Measurable Impact across industries.
                    </p>
                </div>
            </section>
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-40 border-y border-slate-100 dark:border-white/5 bg-slate-50/30 dark:bg-[#0A0D14]">
                    ...
                    <p className="text-xs font-black uppercase tracking-[0.3em] text-slate-400">Loading dynamic case studies...</p>
                </div>
            ) : caseStudies.length === 0 ? (
                <div className="py-40 text-center border-y border-slate-100 dark:border-white/5">
                    <p className="text-slate-400 font-medium">Coming soon: Our latest success stories.</p>
                </div>
            ) : (
            ...
            <section className="py-24 bg-brand-dark dark:bg-black text-white overflow-hidden">
                <div className="container-custom mb-16">
                    <h2 className="text-xs font-bold text-brand-accent uppercase tracking-[0.4em] mb-4">Visual Evidence</h2>
                    <h3 className="text-4xl font-bold font-outfit">Transformation Previews</h3>
                </div>
                <div className="container-custom">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {[
                            ...
                            <div key={i} className="group relative aspect-[4/5] rounded-xl overflow-hidden bg-white/5 border border-white/10">
                                <div className="absolute inset-0 flex items-center justify-center">
                                    <span className="text-white/20 font-outfit font-black text-6xl">{i + 1}</span>
                                </div>
                                ...
                                <div className="absolute bottom-8 left-8">
                                    <span className="text-[10px] text-brand-accent font-bold uppercase tracking-widest block mb-2">{item.tag}</span>
            ...
            <section className="py-24 bg-white dark:bg-[#0A0D14] text-center">
                <div className="container-custom max-w-4xl">
                    <h2 className="text-5xl lg:text-7xl font-bold text-brand-dark dark:text-white mb-12 font-outfit tracking-tighter">Numbers That Define <br />
                        <span className="text-brand-primary">Performance.</span></h2>
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
                        {[
                            ...
                            <div key={i} className="flex flex-col items-center">
                                <span className="text-4xl lg:text-5xl font-black text-brand-dark dark:text-white mb-4">{stat.val}</span>
                                <span className="text-xs font-black text-brand-primary uppercase tracking-[0.2em]">{stat.label}</span>
                            </div>
                        ))}
                    </div>
                </div>
